...
export default function Login() {
  ...
  async function onSubmit(values: z.infer<typeof AuthenticationSchemas.login>) {
    try {
      ...
      if (!response.success) {
        setErrorMessage(t('auth.login.incorrectCredentials'));
      } else {
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    try {
      ...
      if (response.success) {
        ...
        setResetError(t('auth.errors.somethingWentWrongDot'));
      }
    } catch {
      setResetStatus('error');
      setResetError(t('auth.errors.somethingWentWrongDot'));
  ...
  return (
    <>
      <NextSeo title={t('auth.login.seoTitle')} />
      <div
      ...
      >
                      ...
                      <>
                        ...
                        <div className="relative">
                          ...
                          <div className="relative flex justify-center text-xs uppercase">
                            <span className="bg-white px-2 text-neutral-400 tracking-wider">{t('auth.oauth.or')}</span>
                          </div>
                        </div>
                      </>
                    )}
                        ...
                        render={({field}) => (
                          <FormItem>
                            <div className="flex items-center gap-2">
                              ...
                              {lastUsed === 'email' && (
                                <span className="text-[10px] text-neutral-400">{t('auth.oauth.lastUsed')}</span>
                              )}
                            </div>
                            <FormControl>
                              <Input placeholder={t('auth.fields.emailPlaceholder')} autoFocus {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>{t('auth.login.reset.title')}</DialogTitle>
              <DialogDescription>{t('auth.login.reset.description')}</DialogDescription>
            </DialogHeader>
            <form
              onSubmit={e => {
