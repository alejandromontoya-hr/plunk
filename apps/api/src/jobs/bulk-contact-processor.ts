...
async function applyBatch(
  ...
  switch (operation) {
    ...
    case 'set-data': {
      if (!data?.field) {
        throw new Error('data.field is required for set-data');
      }
      const r = await ContactService.bulkSetData(projectId, ids, data.field, data.value);
      return {changed: r.updated, unchanged: 0};
    }
  }
}
...
export function createBulkContactWorker() {
  const worker = new Worker<BulkContactActionJobData>(
    ...
    async (job: Job<BulkContactActionJobData>) => {
      const {projectId, operation, selector, segmentId, data} = job.data;
      ...
      if (selector.mode === 'ids') {
        ...
        for (let i = 0; i < contactIds.length; i += BATCH_SIZE) {
          ...
          try {
            const {changed, unchanged} = await applyBatch(projectId, operation, batchIds, segmentId, data);
