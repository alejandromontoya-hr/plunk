...
export class Contacts {
  ...
  /**
   * POST /contacts/bulk-set-data
   * Queue overwriting a custom field (Contact.data.<field> = value) on the selected
   * contacts. Body: { field, value, ...bulkAction selector (ids | query) }
   */
  @Post('bulk-set-data')
  @Middleware([requireAuth, requireEmailVerified])
  @CatchAsync
  public async bulkSetData(req: Request, res: Response, _next: NextFunction) {
    const auth = res.locals.auth;

    const {field, value} = (req.body ?? {}) as {field?: unknown; value?: unknown};
    if (typeof field !== 'string' || field.trim().length === 0 || field.length > 100) {
      return res.status(400).json({error: 'field is required (1-100 chars)'});
    }
    if (typeof value !== 'string' || value.length > 500) {
      return res.status(400).json({error: 'value is required (max 500 chars)'});
    }
    ...
    const job = await QueueService.queueBulkContactAction(auth.projectId!, selector, 'set-data', undefined, {
      field: field.trim(),
      value,
    });
    return res.status(202).json({message: 'Bulk set-data queued successfully', jobId: job.id});
  }
